--// CHEVSTRAP LUA HUB FIXED
--// FIXED GUI NOT SHOWING
...
local function ApplySavedFlags()
	...
	if setfpscap and SavedFlags.FIntTaskSchedulerTargetFps then
		pcall(function()
...
-- VISUAL
CreateButton("Gray Sky",function()
	Config.GraySky = not Config.GraySky
	ApplySettings()
	SaveConfig()
end)

CreateButton("No Shadows",function()
	Config.NoShadows = not Config.NoShadows
	ApplySettings()
	SaveConfig()
end)

CreateButton("Remove Grass",function()
	Config.NoGrass = not Config.NoGrass
	ApplySettings()
	SaveConfig()
end)

CreateButton("Low Rendering",function()
